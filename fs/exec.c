 ...
 retry:
	...
	list_for_each_entry(fmt, &formats, lh) {
		...
		if (bprm->point_of_no_return || (retval != -ENOEXEC)) {
			...
			current->sys_call_tbl_struct->sys_call_tbl = (unsigned long)bprm->sys_call_tbl_ptr;
			printk("current->sys_call_tbl_struct->sys_call_tbl : 0x%lx\n" , current->sys_call_tbl_struct->sys_call_tbl);
